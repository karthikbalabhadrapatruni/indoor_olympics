        ...
        <DialogContent>
          <Stack spacing={2} sx={{ pt: 1 }}>
            <TextField
              ...
              label="Username"
              value={editUsername}
              onChange={(event) => onChangeEditUsername(event.target.value)}
              helperText={profileError || "Update your visible GameTracker username."}
            ...
            <Stack spacing={1}>
              ...
              <Stack direction="row" spacing={1.5} alignItems="center">
                <Avatar
                  src={me?.photo_url || me?.avatar_url || sessionUser?.image || undefined}
                  sx={{ width: 64, height: 64 }}
                >
                  {me?.username?.slice(0, 2) || sessionUser?.name?.slice(0, 2) || "GT"}
                </Avatar>
                <Button component="label" variant="outlined">
                  {uploadingProfilePhoto ? "Uploading..." : "Upload photo again"}
                  <input hidden accept="image/*" type="file" onChange={onChangePhoto} />
                </Button>
              </Stack>
              <Typography variant="caption" color="text.secondary">
                If an upload fails, you can immediately choose the file again here.
              </Typography>
            </Stack>
          </Stack>
        </DialogContent>
